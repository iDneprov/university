...
int main(int argc, char *argv[])
{
    ...
    // распараллеливание на процессоре одной машины
    //  shared -- аналог глобальной памяти, а private -- переменная, котрая будет одна для всех процессов
    ...
    // прописываем значения на границах
      ...
      #pragma omp parallel for private(z, y)
        for(z = 0; z < b[Z]; z++)
            for (y = 0; y < b[Y]; y++) {
        ...
        #pragma omp parallel for private(z, x)
        for (z = 0; z < b[Z]; z++)
            for (x = 0; x < b[X]; x++) {
                ...
                for (j = 0; j < p[Y]; j++) // цикл по процессам
                    for (y = 0; y < b[Y]; y++) { // цикл по слоям
                        for (i = 0; i < p[X]; i++) { // цикл по процессам
                            if (PROCESS_3D_TO_1D(i, j, k)) { // строка нулевому процессу не пренадлежит, то он её получает и пишет
                                MPI_Recv(buffer, b[X], MPI_DOUBLE, PROCESS_3D_TO_1D(i, j, k), 1, MPI_COMM_WORLD, &status_mpi);
                                vector_print(buffer, b[X]);
                            } else // если принадлежит, то пишет сразу

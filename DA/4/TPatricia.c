...
TPatriciaNode CreateNewTPNode (char* key, ABCIdx value, TPatriciaNode sibling, TPatriciaNode child, bool isEoW) {
...
bool WriteValue (FILE* f, ABCIdx value) {
...
bool ReadValue (FILE* f, ABCIdx* value) {
    char* binValue = (char *) value;
    int c;
    for (int i = 0; i < sizeof(ABCIdx); i++) {
        c = getc(f);
